...
public class APIInventoryController extends APIController {
    ...
    public ResponseEntity updateInventory ( @RequestBody final Inventory inventory ) {
        ...
        for ( int i = 0; i < inventory.getIngredients().size(); i++ ) {
            final String ingName = inventory.getIngredients().get( i ).getName();
            final int ingAmt = inventory.getIngredients().get( i ).getAmount();
            final Ingredient ing = inventoryCurrent.getIngredient( ingName );
            if ( null == ing ) {
                inventoryCurrent.addIngredient( ingName, ingAmt );
            }
            else {
                inventoryCurrent.updateInventory( ing, ingAmt );
            }
        }
